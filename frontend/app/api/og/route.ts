...
export async function GET(req: NextRequest) {
  ...
  let highlight = ''
  try {
    const res = await fetch(endpoint, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ query: gql, variables: { id: scenarioId } }) })
    const js = await res.json()
    const s = js?.data?.shareCard
    if (s) { title = s.title; deficit = s.deficit; debtPct = s.debtDeltaPct; highlight = s.highlight }
  } catch {}
  const deficitText = `${deficit >= 0 ? '+' : ''}${(deficit/1e9).toFixed(2)} B€`
...
<svg xmlns="http://www.w3.org/2000/svg" width="1200" height="630">
  ...
  <text x="60" y="120" fill="#ffffff" font-size="52" font-family="Arial, Helvetica, sans-serif">${title}</text>
  <text x="60" y="180" fill="#9ad" font-size="24" font-family="Arial, Helvetica, sans-serif">Deficit (Y1)</text>
  <text x="60" y="210" fill="#fff" font-size="36" font-family="Arial, Helvetica, sans-serif">${deficitText}</text>
  <text x="60" y="260" fill="#9ad" font-size="24" font-family="Arial, Helvetica, sans-serif">Debt Δ (H)</text>
  <text x="60" y="290" fill="#fff" font-size="28" font-family="Arial, Helvetica, sans-serif">${debtPct.toFixed(2)} pp</text>
  <text x="60" y="340" fill="#9ad" font-size="24" font-family="Arial, Helvetica, sans-serif">Highlight</text>
  <text x="60" y="370" fill="#fff" font-size="22" font-family="Arial, Helvetica, sans-serif">${highlightText}</text>
  <rect x="60" y="420" width="1000" height="24" fill="url(#g)"/>
  <text x="60" y="480" fill="#9ad" font-size="18" font-family="Arial, Helvetica, sans-serif">Methods v0 · Policy catalog v0</text>
  <text x="60" y="510" fill="#9ad" font-size="18" font-family="Arial, Helvetica, sans-serif">citizenbudgetlab.org</text>
</svg>`
...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    const s = js?.data?.shareCard
    if (s) {
      title = s.title; deficit = s.deficit; debtPct = s.debtDeltaPct; highlight = s.highlight; resolutionPct = s.resolutionPct || 0; masses = s.masses || {}; eu3 = s.eu3 || 'info'; eu60 = s.eu60 || 'info'
    }
  } catch {}
...
<svg xmlns=\"http://www.w3.org/2000/svg\" width=\"1200\" height=\"630\">
  ...
  <text x=\"300\" y=\"150\" fill=\"#9ad\" font-size=\"22\" font-family=\"Arial, Helvetica, sans-serif\">Debt Δ (H)</text>
