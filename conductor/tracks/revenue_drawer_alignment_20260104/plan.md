...
- [x] Task: Audit `data/policy_levers.yaml` and `data/lego_pieces.json` to identify current revenue reforms and categories. [commit: 342b02a]
- [x] Task: Adapt `tools/research_policy.py` to handle revenue-specific search queries and data structures. [commit: 9600830]
- [x] Task: Execute the research pipeline for all revenue reforms to gather metadata, vigilance points, and source links. [commit: b94c01c]
- [x] Task: Update `data/policy_levers.yaml` with the newly researched metadata. [commit: b94c01c]
- [x] Task: Map revenue reforms to their parent categories in `data/lego_pieces.json` or `data/policy_levers.yaml` (using `target_revenue_category_id`). [commit: b94c01c]
